...
// JFrame Credit: https://cs.lmu.edu/~ray/notes/javanetexamples/#chat
public class Client 
{
	private int port;
	private String ip;
	...
	private static String[] emoticonKeywords = {"oWo", "smile", "<3", "zzz", "wut", "shite", "joecup", "ale"};
	private static int[] codePoints = {0x1F60B, 0x1F600, 0x2764, 0x1F634, 0x1F610, 0x1F4A9, 0x2615, 0x1F37A};

	public Client()
	{
		this.ip = getIP();
		try {this.port = Integer.parseInt(getPort()); }
		catch(NumberFormatException e) { this.port = 7777;}
	...
	private void run() throws IOException
	{
		...
		{
			String input;
			try {input = recieving.readLine(); }
			catch(SocketException e) {System.out.println("Connection closed."); messageBox.append("Connection closed.\n"); break;}
			...
			{
				for(int i = 0; i < emoticonKeywords.length; i++)
					if(input.contains(emoticonKeywords[i]))
					{
						char[] ar = Character.toChars(codePoints[i]);
						input = input.replaceAll(emoticonKeywords[i], new String(ar, 0, ar.length));
					}
